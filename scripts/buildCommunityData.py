...
def buildAssignmentOutputs(assignments: list[dict], articles: list[dict], sourceMode: str, sourceFile: str, latest: datetime, generatedAt: str) -> tuple[dict, list[tuple[str, dict]]]:
    articleByPmid = {str(article.get("pmid") or ""): article for article in articles}
    ...
    recentItems = [
        assignmentPublicItem(item, articleByPmid.get(item["pmid"], {}))
        for item in assignments
        if withinWindow(articleByPmid.get(item["pmid"], {}), recentCutoff)
    ]
    ...
    indexPayload = {
        ...
        "loading_note": "首屏只加载 taxonomy/cards/weekly/audit；全量 assignments 已按社区拆分为 communityAssignments-*.js 分片。",
    }
    return indexPayload, shardPayloads
...
def main() -> int:
    ...
    assignmentIndexPayload, assignmentShardPayloads = buildAssignmentOutputs(assignments, articles, sourceMode, sourceFile, latest, generatedAt)
