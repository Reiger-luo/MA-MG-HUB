...
conceptDefs = [
    ...
    {
        "id": "cytokineSignaling",
        "title": "Cytokine Signaling",
        "type": "mechanism",
        "summary": "细胞因子、白介素、干扰素、趋化因子和炎症信号相关机制研究。",
        "patterns": [r"\bcytokine\b", r"\binterleukin\b", r"\bIL-\d+\b", r"\binterferon\b", r"\bchemokine\b", r"\bTNF\b"],
    },
    {
        "id": "immuneCellSignature",
        "title": "Immune Cell Signature",
        "type": "mechanism",
        "summary": "免疫细胞谱系、T/B 细胞表型和单细胞免疫特征相关机制研究。",
        "patterns": [r"\bimmune cell", r"\bT-cell\b", r"\bT cell\b", r"\bsingle-cell\b", r"\bcellular signature\b", r"\blymphocyte subset", r"\bTreg\b", r"\bTh17\b"],
    },
    {
    ...
    },
    {
        ...
        "title": "Azathioprine",
        "type": "drug",
        "summary": "硫唑嘌呤相关 MG 维持治疗、激素减量和长期安全性证据。",
        "patterns": [r"\bazathioprine\b"],
    },
    {
        "id": "mycophenolate",
        "title": "Mycophenolate",
        "type": "drug",
        "summary": "麦考酚酸酯/麦考酚酸相关 MG 维持治疗、激素减量和长期安全性证据。",
        "patterns": [r"\bmycophenolate\b", r"\bmycophenolic acid\b", r"\bmycophenolate mofetil\b"],
    },
    {
...
def loadArticles(inputPath: Path) -> tuple[list[dict], str]:
    ...
    if recentJsPath.exists():
        return load_js_global(recentJsPath, "MG_LITERATURE_DATA"), str(recentJsPath.relative_to(projectPath))
...
def writeJs(data: dict, outputPath: Path, sourceLabel: str) -> None:
    ...
    atomic_write_js_global(outputPath, "MG_KNOWLEDGE_GRAPH", data, header)
...
def writeGraphHealthJs(data: dict) -> None:
    ...
    atomic_write_js_global(graphHealthJsPath, "MG_GRAPH_HEALTH", data, header)
