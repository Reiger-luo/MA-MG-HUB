...
def test_landscape_exposes_chictr_registry_signals_without_oxford_grading():
    html = (ROOT / "pages" / "landscape.html").read_text(encoding="utf-8")
    js = (ROOT / "assets" / "landscape.js").read_text(encoding="utf-8")
    assert '<script src="../data/source-signals.js"></script>' in html
    assert 'id="chinaTrialRegistrySignals"' in html
    assert "window.MG_SOURCE_SIGNALS" in js
    assert "ChiCTR" in js
    block = js[js.index("function renderChinaTrialRegistrySignals"):js.index("function renderGuidelineSlot")]
    assert "evidence_level" not in block
    assert "escapeHref" in block
